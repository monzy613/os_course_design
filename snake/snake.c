...
void wait(int i, int timer, char *last_direction, struct snakeNode *snakeHead);
...
void detectingDying(struct snakeNode *head, struct snakeNode *tail);
void delay();
...
restart:
	...
	for (;;) {
		...
		wait(1, timer, last_direction, head);
	}
...
int headHitWall(int x, int y) {
	if (x < 6 || x > 185 || y < 25 || y > 180) {
		return 1;
	} else {
		return 0;
	}
...
void detectingDying(struct snakeNode *head, struct snakeNode *tail) {
	while (tail != head) {
		if ( (head->posX == tail->posX && head->posY == tail->posY) || headHitWall(head->posX, head->posY) == 1) {
			isPause = 1;
			api_putstrwin(globalWin, 50, 50, 1, 9, "Game Over");
		}
		tail = tail->preNode;
	}
}
...
void wait(int i, int timer, char *last_direction, struct snakeNode *snakeHead)
{
	...
	for (;;) {
		...
		if (j == 0x0a) {
			api_putstrwin(globalWin, 50, 50, 0, 9, "Game Over");
			isPause = 0;
		}
